...
<script>
    ...
    var untukTambahDanUbahData = function() {
        $(document).on('submit', '#form-add-upd-head', function(e) {
            ...
            $('#total_akhir').attr('required', 'required');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        swal({
                            ...
                            window.location = '<?= pelayan_url() ?>pemesanan/detail/' + btoa(response.id);
                        });
    ...
    var untukTabelTempporaryPemesanan = function() {
        tabelDataDt = $('#tabel-temp-pemesanan').DataTable({
            ...
            pageLength: 10,
            ajax: '<?= pelayan_url() ?>pemesanan/get_data_temp_dt',
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        <div class="button-icon-btn button-icon-btn-cl">
                            <button type="button" id="btn-upd" data-id="` + full.id_keranjang + `" class="btn btn-sm btn-info" data-toggle="modal" data-target="#modal-add-upd"><i class="fa fa-pencil"></i>&nbsp;Ubah</button>&nbsp;
                            <button type="button" id="btn-del" data-id="` + full.id_keranjang + `" class="btn btn-sm btn-warning"><i class="fa fa-trash"></i>&nbsp;Hapus</button>
                        </div>
                    `;
                    },
                },
    ...
    var untukGetIdData = function() {
        $(document).on('click', '#btn-upd', function() {
            ...
            $.ajax({
                type: "POST",
                url: "<?= pelayan_url() ?>pemesanan/get_temp",
                dataType: 'json',
                data: {
                    id: ini.data('id')
                },
        ...
        $(document).on('click', '#btn-del', function() {
            ...
            swal({
                ...
                if (del) {
                    $.ajax({
                        type: "post",
                        url: "<?= pelayan_url() ?>pemesanan/process_del_temp",
                        dataType: 'json',
                        data: {
                            id: ini.data('id')
                        },
