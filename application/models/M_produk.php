...
class M_produk extends CI_Model
{
    ...
    {
        $result = $this->db->query("SELECT p.id_produk, p.kd_produk, p.nama, p.harga, p.gambar, p.deskripsi, td.diskon, tk.nama AS kategori, IFNULL(( SELECT SUM( s.stock) FROM tb_stock AS s WHERE s.kd_produk = p.kd_produk), 0) AS stock, IFNULL(( SELECT SUM( pd.jumlah ) FROM tb_pemesanan_detail AS pd LEFT JOIN tb_pemesanan AS pe ON pd.kd_pemesanan = pe.kd_pemesanan WHERE pd.kd_produk = p.kd_produk AND pe.status_batal = 'n' ), 0 ) AS jumlah FROM tb_produk AS p LEFT JOIN tb_kategori AS tk ON tk.id_kategori = p.id_kategori LEFT JOIN tb_diskon AS td ON td.id_diskon = p.id_diskon WHERE $column = '$value'");
        return $result;
    }
