...
class M_produk extends CI_Model
{
    ...
    {
        $result = $this->db->query("SELECT p.id_produk, p.id_diskon, p.id_kategori, p.kd_produk, p.nama, p.harga, p.gambar, p.deskripsi, k.nama AS kategori, d.diskon AS diskon,( SELECT SUM( s.stock) FROM tb_stock AS s WHERE s.kd_produk = p.kd_produk) AS stock,( SELECT SUM( pd.jumlah) FROM tb_pemesanan_detail AS pd WHERE pd.kd_produk = p.kd_produk ) AS jumlah,( SELECT SUM( k.jumlah ) FROM tb_keranjang AS k WHERE k.kd_produk = p.kd_produk ) AS jumlah_keranjang FROM tb_produk AS p LEFT JOIN tb_kategori AS k ON k.id_kategori = p.id_kategori LEFT JOIN tb_diskon AS d ON d.id_diskon = p.id_diskon");
        return $result;
    }
