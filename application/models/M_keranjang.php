...
class M_keranjang extends CI_Model
{
    ...
    {
        $result = $this->db->query("SELECT tk.id_keranjang, tk.id_users, tk.kd_produk, tk.jumlah AS jumlah_keranjang,( SELECT SUM( ts.stock) FROM tb_stock AS ts WHERE ts.kd_produk = tp.kd_produk ) AS stock,( SELECT SUM( tpd.jumlah ) FROM tb_pemesanan_detail AS tpd WHERE tpd.kd_produk = tp.kd_produk ) AS jumlah, tp.nama, tp.gambar, tp.jenis,( CASE WHEN tp.jenis = 'cake' THEN IFNULL(( SELECT SUM( pt.harga ) FROM tb_produk_topper AS pt RIGHT JOIN tb_keranjang AS k ON pt.kd_topper = k.kd_produk WHERE pt.harga IS NOT NULL ), 0 ) + tp.harga ELSE tp.harga END ) AS harga, ( CASE WHEN tp.jenis = 'cake' THEN IFNULL(( SELECT SUM(( k.jumlah * pt.harga )) AS harga FROM tb_produk_topper AS pt RIGHT JOIN tb_keranjang AS k ON pt.kd_topper = k.kd_produk WHERE pt.harga IS NOT NULL ), 0 ) + tk.sub_total ELSE tk.sub_total END ) AS sub_total, d.diskon FROM tb_keranjang AS tk LEFT JOIN tb_produk AS tp ON tk.kd_produk = tp.kd_produk LEFT JOIN tb_diskon AS d ON tp.diskon = d.id_diskon WHERE ( tp.jenis = 'cake' OR tp.jenis = 'dessert' ) AND tk.id_users = '$id_users'");
        return $result;
    }
    ...
    public function getBuyCustomerKeranjangDetail($kd_produk)
    {
        $result = $this->db->query("SELECT pt.kd_topper, pt.nama, pt.harga, k.jumlah FROM tb_produk_topper AS pt RIGHT JOIN tb_keranjang AS k ON pt.kd_topper = k.kd_pasang WHERE k.kd_produk = '$kd_produk'");
        return $result;
    }

    public function getProdukDetailKeranjang($kd_produk)
    {
        $produk        = $this->db->query("SELECT tp.kd_produk, tp.nama, tp.harga, d.diskon FROM tb_produk AS tp LEFT JOIN tb_diskon AS d ON tp.diskon = d.id_diskon WHERE tp.kd_produk = '$kd_produk'");
        $produk_topper = $this->db->query("SELECT tpt.kd_topper AS kd_produk, tpt.nama, tpt.harga FROM tb_produk_topper AS tpt WHERE tpt.kd_topper = '$kd_produk'");
        ...
        if ($num_produk === 1) {
            ...
            foreach ($produk_topper->result() as $row) {
                $result = [
                    'kd_produk' => $row->kd_produk,
                    'nama'      => $row->nama,
                    'harga'     => $row->harga,
