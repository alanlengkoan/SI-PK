...
class M_pemesanan extends CI_Model
{
    ...
    {
        if ($jenis === 'all') {
            $result = $this->db->query("SELECT tu.nama AS customer, tp.kd_produk, tpe.kd_pemesanan, DATE_FORMAT( tpe.tgl_pemesanan, '%d-%m-%Y') AS tgl_pemesanan, DATE_FORMAT( tpe.tgl_pemesanan, '%H:%i:%s' ) AS jam_pemesanan, SUM( tpd.sub_total ) AS total,( SELECT SUM( pe.jumlah_transfer ) FROM tb_transfer AS pe WHERE pe.kd_pemesanan = tpe.kd_pemesanan ) AS transfer, ( SELECT SUM( c.jumlah_bayar ) FROM tb_cod AS c WHERE c.kd_pemesanan = tpe.kd_pemesanan ) AS bayar FROM tb_pemesanan AS tpe LEFT JOIN tb_pemesanan_detail AS tpd ON tpe.kd_pemesanan = tpd.kd_pemesanan LEFT JOIN tb_produk AS tp ON tpd.kd_produk = tp.kd_produk LEFT JOIN tb_users AS tu ON tpe.id_users = tu.id_users WHERE tp.jenis IN ( 'cake', 'dessert' ) AND DATE( tpe.tgl_pemesanan ) BETWEEN '$tgl_awal' AND '$tgl_akhir' GROUP BY tu.nama, tp.kd_produk, tpe.kd_pemesanan, tpe.tgl_pemesanan");
        } else {
            $result = $this->db->query("SELECT tu.nama AS customer, tp.kd_produk, tpe.kd_pemesanan, DATE_FORMAT( tpe.tgl_pemesanan, '%d-%m-%Y') AS tgl_pemesanan, DATE_FORMAT( tpe.tgl_pemesanan, '%H:%i:%s' ) AS jam_pemesanan, SUM( tpd.sub_total ) AS total,( SELECT SUM( pe.jumlah_transfer ) FROM tb_transfer AS pe WHERE pe.kd_pemesanan = tpe.kd_pemesanan ) AS transfer, ( SELECT SUM( c.jumlah_bayar ) FROM tb_cod AS c WHERE c.kd_pemesanan = tpe.kd_pemesanan ) AS bayar FROM tb_pemesanan AS tpe LEFT JOIN tb_pemesanan_detail AS tpd ON tpe.kd_pemesanan = tpd.kd_pemesanan LEFT JOIN tb_produk AS tp ON tpd.kd_produk = tp.kd_produk LEFT JOIN tb_users AS tu ON tpe.id_users = tu.id_users WHERE tp.jenis = '$jenis' AND DATE( tpe.tgl_pemesanan ) BETWEEN '$tgl_awal' AND '$tgl_akhir' GROUP BY tu.nama, tp.kd_produk, tpe.kd_pemesanan, tpe.tgl_pemesanan");
        }
        return $result;
    }
    ...
    {
        if ($jenis === 'all') {
            $result = $this->db->query("SELECT tu.nama AS customer, tp.kd_produk, tpe.kd_pemesanan, DATE_FORMAT( tpe.tgl_pemesanan, '%d-%m-%Y') AS tgl_pemesanan, DATE_FORMAT( tpe.tgl_pemesanan, '%H:%i:%s' ) AS jam_pemesanan, SUM( tpd.sub_total ) AS total,( SELECT SUM( pe.jumlah_transfer ) FROM tb_transfer AS pe WHERE pe.kd_pemesanan = tpe.kd_pemesanan ) AS transfer, ( SELECT SUM( c.jumlah_bayar ) FROM tb_cod AS c WHERE c.kd_pemesanan = tpe.kd_pemesanan ) AS bayar FROM tb_pemesanan AS tpe LEFT JOIN tb_pemesanan_detail AS tpd ON tpe.kd_pemesanan = tpd.kd_pemesanan LEFT JOIN tb_produk AS tp ON tpd.kd_produk = tp.kd_produk LEFT JOIN tb_users AS tu ON tpe.id_users = tu.id_users WHERE tp.jenis IN ( 'cake', 'dessert' ) AND MONTH ( tpe.tgl_pemesanan ) = '$bulan' AND YEAR ( tpe.tgl_pemesanan ) = '$tahun' GROUP BY tu.nama, tp.kd_produk, tpe.kd_pemesanan, tpe.tgl_pemesanan");
        } else {
            $result = $this->db->query("SELECT tu.nama AS customer, tp.kd_produk, tpe.kd_pemesanan, DATE_FORMAT( tpe.tgl_pemesanan, '%d-%m-%Y') AS tgl_pemesanan, DATE_FORMAT( tpe.tgl_pemesanan, '%H:%i:%s' ) AS jam_pemesanan, SUM( tpd.sub_total ) AS total,( SELECT SUM( pe.jumlah_transfer ) FROM tb_transfer AS pe WHERE pe.kd_pemesanan = tpe.kd_pemesanan ) AS transfer, ( SELECT SUM( c.jumlah_bayar ) FROM tb_cod AS c WHERE c.kd_pemesanan = tpe.kd_pemesanan ) AS bayar FROM tb_pemesanan AS tpe LEFT JOIN tb_pemesanan_detail AS tpd ON tpe.kd_pemesanan = tpd.kd_pemesanan LEFT JOIN tb_produk AS tp ON tpd.kd_produk = tp.kd_produk LEFT JOIN tb_users AS tu ON tpe.id_users = tu.id_users WHERE tp.jenis = '$jenis' AND MONTH ( tpe.tgl_pemesanan ) = '$bulan' AND YEAR ( tpe.tgl_pemesanan ) = '$tahun' GROUP BY tu.nama, tp.kd_produk, tpe.kd_pemesanan, tpe.tgl_pemesanan");
        }
        return $result;
    }
    ...
    {
        if ($jenis === 'all') {
            $result = $this->db->query("SELECT tu.nama AS customer, tp.kd_produk, tpe.kd_pemesanan, DATE_FORMAT( tpe.tgl_pemesanan, '%d-%m-%Y') AS tgl_pemesanan, DATE_FORMAT( tpe.tgl_pemesanan, '%H:%i:%s' ) AS jam_pemesanan, SUM( tpd.sub_total ) AS total,( SELECT SUM( pe.jumlah_transfer ) FROM tb_transfer AS pe WHERE pe.kd_pemesanan = tpe.kd_pemesanan ) AS transfer, ( SELECT SUM( c.jumlah_bayar ) FROM tb_cod AS c WHERE c.kd_pemesanan = tpe.kd_pemesanan ) AS bayar FROM tb_pemesanan AS tpe LEFT JOIN tb_pemesanan_detail AS tpd ON tpe.kd_pemesanan = tpd.kd_pemesanan LEFT JOIN tb_produk AS tp ON tpd.kd_produk = tp.kd_produk LEFT JOIN tb_users AS tu ON tpe.id_users = tu.id_users WHERE tp.jenis IN ( 'cake', 'dessert' ) AND YEAR ( tpe.tgl_pemesanan ) = '$tahun' GROUP BY tu.nama, tp.kd_produk, tpe.kd_pemesanan, tpe.tgl_pemesanan");
        } else {
            $result = $this->db->query("SELECT tu.nama AS customer, tp.kd_produk, tpe.kd_pemesanan, DATE_FORMAT( tpe.tgl_pemesanan, '%d-%m-%Y') AS tgl_pemesanan, DATE_FORMAT( tpe.tgl_pemesanan, '%H:%i:%s' ) AS jam_pemesanan, SUM( tpd.sub_total ) AS total,( SELECT SUM( pe.jumlah_transfer ) FROM tb_transfer AS pe WHERE pe.kd_pemesanan = tpe.kd_pemesanan ) AS transfer, ( SELECT SUM( c.jumlah_bayar ) FROM tb_cod AS c WHERE c.kd_pemesanan = tpe.kd_pemesanan ) AS bayar FROM tb_pemesanan AS tpe LEFT JOIN tb_pemesanan_detail AS tpd ON tpe.kd_pemesanan = tpd.kd_pemesanan LEFT JOIN tb_produk AS tp ON tpd.kd_produk = tp.kd_produk LEFT JOIN tb_users AS tu ON tpe.id_users = tu.id_users WHERE tp.jenis = '$jenis' AND YEAR ( tpe.tgl_pemesanan ) = '$tahun' GROUP BY tu.nama, tp.kd_produk, tpe.kd_pemesanan, tpe.tgl_pemesanan");
        }
        return $result;
    }
