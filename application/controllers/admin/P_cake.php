...
class P_cake extends MY_Controller
{
    ...
    public function index()
    {
        $data = [
            'title' => 'Cake',
            'content' => 'admin/p_cake/view',
            'css'     => 'admin/p_cake/css/view',
            'js'      => 'admin/p_cake/js/view'
        ];
        // untuk load view
        $this->load->view('admin/base', $data);
    }

    // untuk get data cake by datatable
    public function get_data_cake_dt()
    {
        return $this->m_produk_cake->getAllDataDt();
    }

    // untuk halaman tambah
    public function add()
    {
        $kd_produk = get_kode_urut('tb_produk', 'kd_produk', 'KDP-C-');
        ...
        $data = [
            ...
            'kd_produk' => $kd_produk,
            'content'   => 'admin/p_cake/add',
            'css'       => '',
            'js'        => 'admin/p_cake/js/add'
        ];
        // untuk load view
        $this->load->view('admin/base', $data);
    }
    ...
    public function upd()
    {
        $kd_produk = base64url_decode($this->uri->segment('4'));

        $data = [
            'title'   => 'Cake',
            'content'   => 'admin/p_cake/upd',
            'kd_produk' => $kd_produk,
            'produk'    => $this->m_produk_cake->getProdukDetail($kd_produk),
            'satuan'    => $this->m_satuan->getAll(),
            'diskon'    => $this->m_diskon->getAll(),
            'ukuran'    => $this->m_ukuran->getAll(),
            'css'       => 'admin/p_cake/css/upd',
            'js'        => 'admin/p_cake/js/upd'
        ];
        // untuk load view
        $this->load->view('admin/base', $data);
    }
    ...
    public function process_del()
    {
        ...
        $result_produk = $this->crud->gda('tb_produk', ['kd_produk' => $post['id']]);
        $nm_file_produk = $result_produk['gambar'];
        // menghapus foto yg tersimpan produk
        if ($nm_file_produk !== '' || $nm_file_produk !== null) {
            if (file_exists(upload_path('gambar') . $result_produk['gambar'])) {
                unlink(upload_path('gambar') . $result_produk['gambar']);
            }
        }
